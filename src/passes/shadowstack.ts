 ...
 * size of 16 bytes, and `memory.fill` (`memset` without bulk-memory) beyond.
 * 
 * @license Apache-2.0
 */
...
function matchStack(expr: BinaryenExpressionRef): BinaryenExpressionRef {
  if (_BinaryenExpressionGetId(expr) == ExpressionId.Call && readString(_BinaryenCallGetTarget(expr)) == BuiltinNames.stack) {
...
export class ShadowStackPass extends BinaryenPass {
  ...
  makeStackOffset(offset: i32): BinaryenExpressionRef {
    ...
    var expr = module.block(null, [
      module.global_set(BuiltinNames.stack_ptr,
        module.binary(offset >= 0 ? this.ptrBinaryAdd : this.ptrBinarySub,
          module.global_get(BuiltinNames.stack_ptr, this.ptrType),
      ...
      module.addFunction("~stack_check", NativeType.None, NativeType.None, null,
        module.if(
          module.binary(BinaryOp.LtI32,
            module.global_get(BuiltinNames.stack_ptr, this.ptrType),
            module.global_get(BuiltinNames.stack_base, this.ptrType)
          ),
          this.compiler.makeStaticAbort("stack overflow")
        )
      );
    }
